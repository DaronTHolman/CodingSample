...
#The first argument is the character abbreviated name of a state such as "WA", the second is an outcome name which takes the 
...
#I have placed description of the code below the line the code is written on. 
...
#The function takes three arguments that the user inputs: state, outcome, and num
	...
	#The code reads (or imports) a csv file "outcome-of-care-measures.csv" and assigns the dataset to the variable my_data. 
	#na.strings is used to specify which values in the csv file are missing or not available. stringAsFactors is set to FALSE so that 
	#charachter values are not converted to factors (i.e. values that have text are not saved as numerical data).

	outcomes <- c("heart attack"=11,"heart failure"=17,"pneumonia"=23)
	#This code creates a character vector, gives it the names of the three outcome inputs, and assigns it to the variable outcomes. 
	#The three potential outcome arguments are assigned a number which corresponds to the column number within the inputed csv file 
	...
	if(state %in% my_data$State & outcome %in% names(outcomes)) {
	  ...
	  #and assigns it to the variable my_data2
	}
	else if(state %in% my_data$State == FALSE) { stop("invalid state")
	  #if the user's argument is not valid because the state abbreviated characters do not exist then they are given
	...
	#This code removes all of the records that are missing a piece of data and assigns the new data set to the variable my_data3. 
	#This is so that hospitals that are missing that particular outcome are excluded from the set of hospitals when deciding the ranking. 
	#This code is placed after extracting the columns so that no necessary data is lost (the csv file contains a plethora of other data, 
	#some of which has missing values as well).
	...
	#This code reorders the data in three ways. It first orders them by state in aplphabetical order,
	...
	#(i.e. if hospitals "A" and "F" are tied for best, then hospital "A" is chosen).This reordered 
	#dataset is then assigned to the variable my_data4
	...
	#This list is then assigned to the variable my_data5
	
	my_data6 <- my_data5[[state]]
	#This code extracts a specific data set that the user specifies with the state argument (i.e. if the user specifies
	#"TX" for Texas then that corresponding data set is extracted from the list of 54 data set). The specified data set is
	#then assigned to the variable my_data6.
	...
	#that the user inputted for their num argument and assigns the name of the hospital with that rank to the best variable
	
	best
	#The variable best is then printed to show the name of the hospital with the corresponding rank and state that the user specified.
}

#A format for input follows the format: rankhospital("TX","heart failure",5)